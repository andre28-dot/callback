...
const cantidadEspacios: number =
  solicitarCantidadEspacios();

let resumen: string = "";

for (
  let numeroEspacio: number = 1;
  numeroEspacio <= cantidadEspacios;
  numeroEspacio++
) {
  console.log(
  ...
  const nombreEspacio: string = prompt(
    "Ingrese el nombre del espacio: "
  ).trim();

  const largo: number = solicitarMedida(
    "Ingrese el largo en metros: "
  );

  const ancho: number = solicitarMedida(
    "Ingrese el ancho en metros: "
  );

  const area: number = calcularAreaRectangulo(
    largo,
    ancho
  );
...
console.log(resumen);
console.log(
  `Total de espacios procesados: ${cantidadEspacios}`
);
